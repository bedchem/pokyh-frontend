...
export async function POST(req: NextRequest) {
  ...
  const payload = {
    subject,
    content,
    recipientOption: recipients[0].type,
  ...
  try {
    ...
    if (res.reason === 'error') {
      ...
      return NextResponse.json({ error: `Senden fehlgeschlagen (${res.status})` }, { status: 502 });
    }
    return NextResponse.json(
      { error: 'Kein Senden-Endpunkt gefunden (WEBUNTIS_API_PATH_MSG_SEND in .env setzen).' },
      { status: 502 },
    );
  } catch (e: unknown) {
