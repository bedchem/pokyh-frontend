...
export async function GET() {
  ...
  let appDataDiag = 'session.imageUrl';
  if (!imageUrl) {
    const appData = await fetchAppData(session);
    appDataDiag = appData.ok ? 'appData.ok' : `appData.fail(${'status' in appData ? appData.status : '?'})`;
    imageUrl = appData.ok ? extractImageUrl(appData.json, session.isParent) : undefined;
  }
  console.warn(`[profile-image] user=${session.username} via=${appDataDiag} imageUrl=${imageUrl ?? 'NONE'}`);
  if (!imageUrl) {
  ...
  try {
    ...
    console.warn(`[profile-image] fetch ${target} → ${res.status} ct=${ct}`);
