...
export function SlotBadge({ slot }: { slot: MergedSlot }) {
  ...
  if (d.isCancelled) return <TagChip text={t('cancelled')} color={BRAND.danger} />;
  if (d.isExam) return <TagChip text={t('exam')} color={BRAND.warning} />;
  if (slot.kind === 'replacement') return <TagChip text={t('substitution')} color={BRAND.orange} />;
  if (slot.kind === 'event') return <TagChip text={t('event')} color={BRAND.accent} />;
  return null;
}
...
interface SpecialSpec { Icon: LucideIcon; color: string; title: TimetableKey | null; subtitle: TimetableKey | null }

export function specialDaySpecFor(kind: DayKind): SpecialSpec {
  switch (kind) {
    case 'holiday': return { Icon: Umbrella, color: BRAND.orange, title: 'holiday', subtitle: 'noLessons' };
    case 'weekend': return { Icon: Sun, color: BRAND.success, title: 'weekend', subtitle: 'free' };
    case 'allCancelled': return { Icon: CircleX, color: BRAND.danger, title: 'cancelled', subtitle: 'allCancelled' };
    case 'allReplacement': return { Icon: ArrowLeftRight, color: BRAND.accent, title: 'substitution', subtitle: 'allReplaced' };
    case 'fullDayEvent': return { Icon: CalendarDays, color: BRAND.accent, title: 'event', subtitle: 'allDay' };
    default: return { Icon: CalendarDays, color: BRAND.accent, title: null, subtitle: null };
  }
}
...
export function SpecialDayCard({ kind, compact = false, style, onClick }: {
  ...
  const inner = (
    <div className={s.specialInner}>
      ...
      <p className={s.specialTitle}>{spec.title ? t(spec.title) : ''}</p>
      <p className={s.specialSub}>{spec.subtitle ? t(spec.subtitle) : ''}</p>
    </div>
  );
