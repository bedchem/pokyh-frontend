...
/** "3. Feb. – 8. Feb. 2026" in the given Intl locale. */
export function rangeText(offset: number, tag: string): string {
  const start = mondayOf(offset);
  const end = dateOf(offset, 5);
  return `${formatDate(start, tag, { day: 'numeric', month: 'short' })} – ${formatDate(end, tag, { day: 'numeric', month: 'short', year: 'numeric' })}`;
}
...
export function buildIcs(
  ...
  calendarName: string,
  labels: { lesson: string; teacher: string } = { lesson: 'Stunde', teacher: 'Lehrer' },
): string {
  ...
  for (const e of sorted) {
    ...
    lines.push(`SUMMARY:${icsEscape(e.subjectName || e.note || labels.lesson)}`);
    if (e.roomName) lines.push(`LOCATION:${icsEscape(e.roomName)}`);
    const desc = [e.teacherName ? `${labels.teacher}: ${e.teacherName}` : '', e.examDescription ?? '', e.note ?? ''].filter(Boolean);
