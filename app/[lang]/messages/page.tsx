...
const FOLDERS: { key: MessageFolder; label: MessagesKey }[] = [
...
function parseMessages(json: unknown, t: TFn): MessagePreview[] {
  try {
    ...
    return (arr as Record<string, unknown>[]).map((m) => {
      ...
      const senderName =
        ...
        (recipientLabel ? t('to', { names: recipientLabel }) : t('unknown'));
      const sentDate =
      ...
      return {
        id: m.id as number,
        subject: (m.subject as string) ?? t('noSubject'),
...
function formatMessageDate(dateStr: string, tag: string, yesterday: string): string {
  ...
  try {
    ...
    if (diffDays === 0)
      return formatTime(date, tag, { hour: '2-digit', minute: '2-digit' });
...
export default function MessagesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setMessages(parseMessages(res, t));
    } catch (e: unknown) {
  ...
  async function openDraft(id: number) {
    ...
    try {
      ...
      else setError(t('draftOpenFailed'));
    } finally {
  ...
  async function handleDeleteDraft(id: number) {
    ...
    try {
      ...
      if (e instanceof Error && e.message === 'session_expired') router.replace('/login');
      else setError(t('draftDeleteFailed'));
    } finally {
        ...
        <div className="flex-1 overflow-auto pb-8">
          {loading ? (
            ...
            <EmptyView
              ...
              title={folder === 'sent' ? t('emptySentTitle') : folder === 'drafts' ? t('emptyDraftsTitle') : t('emptyInboxTitle')}
              subtitle={
                folder === 'sent'
                  ? t('emptySentText')
                  : folder === 'drafts'
            ...
            <div style={{ background: 'var(--app-surface)' }} className="fade-in delay-1">
              {messages.map((msg, i) => {
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2">
                        ...
                        <p className="text-xs flex-shrink-0" style={{ color: 'var(--app-text-tertiary)' }}>
                          {formatMessageDate(msg.sentDate, tag, t('yesterday'))}
                        </p>
                      </div>
                ...
                if (!isDraft) {
                  return (
                    <Link key={msg.id} href={localize(`/messages/${msg.id}`)} className="block press-scale" style={{ borderTop }}>
                      {inner}
                    </Link>
                  );
                }
