...
import { useDateLocale, useLocalizeHref, useT } from '@/providers/LocaleProvider';
import { gradesDict } from '@/lib/i18n/dictionaries/grades';
import { commonDict } from '@/lib/i18n/dictionaries/common';

import type { GradesKey } from '@/lib/i18n/dictionaries/grades';
...
function relativeDay(date: number, t: (key: GradesKey, vars?: Record<string, number>) => string): string {
  ...
  if (diff < 7) return t('daysAgo', { n: diff });
  if (diff < 30) return t('weeksAgo', { n: Math.round(diff / 7) });
  if (diff < 365) return t('monthsAgo', { n: Math.round(diff / 30) });
  return t('yearsAgo', { n: Math.round(diff / 365) });
}
...
export default function SubjectDetailPage({
  ...
  const hoverDateText = hoverPoint
    ? (hoverPoint.isCustom ? t('ownGrade') : fmtDateShort(hoverPoint.date))
    : '';
  ...
  if (!subject) {
    return (
      <AuthGuard>
        <div className="subject-state">
          <ErrorView message={t('subjectNotFound')} onRetry={() => router.push(localize('/grades'))} />
        </div>
      </AuthGuard>
    );
  }
        ...
        <div className="subject-page">
          <header className="page-head">
            <div className="head-left">
              <button
                type="button"
                onClick={() => router.push(localize('/grades'))}
                className="back-btn"
                aria-label={tc('back')}
              >
                <ChevronLeft size={20} />
              ...
              <div className="head-text">
                <p className="head-eyebrow">{t('subject')}</p>
          ...
          <section className="kpis">
            <div className="card kpi-main">
              <div className="card-hd">
                <div>
                  <div className="card-title">{t('average')}</div>
                  <div className="card-sub">{liveValues.length === 1 ? t('gradeOne', { n: 1 }) : t('gradeMany', { n: liveValues.length })}{hasDraft ? ` · ${t('simulation')}` : ''}</div>
                </div>
              ...
              <div className="kpi-foot">
                {hasDraft ? (
                  <span>{t('teacherAvg')} <span className="mono">{fmtNum(teacherAvg, 2)}</span></span>
                ) : (
                  <span>{liveValues.length > 0 ? t('best', { grade: formatMark(Math.max(...liveValues)) }) : '—'}</span>
                )}
                <span className="trend-mini">
                  <TrendIconElem size={14} />
                  {trendDirection === 'up' ? t('rising') : trendDirection === 'down' ? t('falling') : t('stable')}
                </span>
              </div>
            </div>
            ...
            <div className="card">
              <div className="card-hd">
                <div>
                  <div className="card-title">{t('ratioTitle')}</div>
                  <div className="card-sub">{t('ratioSub')}</div>
                </div>
            ...
            <div className="card target-card">
              <div className="card-hd">
                <div>
                  <div className="card-title">{t('whatDoINeed')}</div>
                  <div className="card-sub">{t('targetCalc')}</div>
                </div>
              ...
              <div className="target-input-row">
                ...
                {(() => {
                  ...
                  if (targetResult.status === 'reached') {
                    return <span className="target-result v-positive">{t('reached')}</span>;
                  }
                  if (targetResult.status === 'impossible') {
                    return <span className="target-result v-critical">{t('impossible')}</span>;
                  }
              ...
              <p className="target-hint">
                {(() => {
                  if (!targetResult) return t('hintEnter');
                  if (targetResult.status === 'reached') return t('hintReached');
                  if (targetResult.status === 'impossible') return t('hintImpossible');
                  if (targetResult.count === 1) return t('hintNext');
                  return t('hintMany', { n: targetResult.count, grade: fmtNum(targetResult.needed, 2) });
                })()}
              </p>
            </div>
          </section>
          ...
          <section className="trend-section">
            ...
            <div className="trend-card">
              {chronological.length === 0 ? (
                <div className="trend-empty">{t('noGradesYet')}</div>
              ) : (
          ...
          <section className="grid-section">
            <div className="card list-card">
              <div className="section-hd inline">
                <div>
                  <h2>{t('gradeList')}</h2>
                  <p>{t('activeExcluded', { active: teacherRows.filter((r) => !r.removed).length, excluded: teacherRows.filter((r) => r.removed).length })}</p>
                </div>
              </div>
              <div className="grade-list">
                {teacherRows.length === 0 ? (
                  <div className="empty-list">{t('noGradesRecorded')}</div>
                ) : (
                  teacherRows.map((row) => (
                    <div key={row.id} className={`grade-item ${row.removed ? 'is-removed' : ''}`}>
                      <div className="grade-meta">
                        ...
                        <span className="grade-when">
                          {fmtDateShort(row.date)} · {relativeDay(row.date, t)}
                        </span>
                      </div>
            ...
            <div className="card calc-card">
              ...
              <div className="add-row">
                ...
                />
                <button type="button" onClick={addCustomGrade} aria-label={t('addGrade')}>
                  <Plus size={15} />
                </button>
              </div>
              ...
              <div className="custom-list">
                {draft.customGrades.length === 0 ? (
                  <div className="empty-list small">{t('noOwnGrades')}</div>
                ) : (
              ...
              <div className="quick-add">
                <span className="quick-label">{t('quickTest')}</span>
          ...
          <section className="meta-foot">
            <span>
              {t('lastGrade')} <strong>{teacherRows[0] ? fmtUntisDateLong(teacherRows[0].date, tag) : '—'}</strong>
            </span>
            <Link href={localize('/grades')} className="all-link">{t('allSubjects')}</Link>
          </section>
        </div>
      </div>
