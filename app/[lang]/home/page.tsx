...
import { dateNumOf, nextUpcomingExam, schoolMinuteNow } from '@/app/[lang]/timetable/timetable-logic';
import { useDateLocale, useLocale, useLocalizeHref, useT } from '@/providers/LocaleProvider';
import { homeDict, type HomeKey } from '@/lib/i18n/dictionaries/home';
import { messagesDict } from '@/lib/i18n/dictionaries/messages';
import type { Locale } from '@/lib/i18n/locale';
import { formatDate, formatTime } from '@/lib/i18n/dateLocale';

type TFn = (key: HomeKey, vars?: Record<string, string | number>) => string;
...
function formatMessageDate(dateStr: string, tag: string, yesterday: string): string {
  if (!dateStr) return '';
  try {
    const date = new Date(dateStr);
    const diffDays = Math.floor((Date.now() - date.getTime()) / 86400000);
    if (diffDays === 0) return formatTime(date, tag, { hour: '2-digit', minute: '2-digit' });
    if (diffDays === 1) return yesterday;
    if (diffDays < 7) return formatDate(date, tag, { weekday: 'short' });
    return formatDate(date, tag, { day: '2-digit', month: '2-digit' });
  } catch { return ''; }
}
...
function parseMessagesResult(json: unknown, unknownLabel: string, noSubject: string): MessagePreview[] {
  try {
    ...
    return (arr as Record<string, unknown>[]).map((m) => {
      ...
      const senderName = (sender?.displayName as string) ?? (sender?.name as string) ?? (m.senderName as string) ?? unknownLabel;
      const sentDate = (m.sentDateTime as string) ?? (m.sentDate as string) ?? (m.date as string) ?? '';
      return { id: m.id as number, subject: (m.subject as string) ?? noSubject, contentPreview: (m.contentPreview as string) ?? '', senderName, senderId: (sender?.userId as number) ?? 0, sentDate, isRead: (m.isRead as boolean) ?? true, hasAttachments: (m.hasAttachments as boolean) ?? false };
    });
  } catch { return []; }
}

function formatExamDate(d: number, tag: string): string {
  const s = d.toString();
  const date = new Date(parseInt(s.slice(0, 4)), parseInt(s.slice(4, 6)) - 1, parseInt(s.slice(6, 8)));
  return formatDate(date, tag, { weekday: 'short', day: 'numeric', month: 'short' });
}

function daysUntilLabel(d: number, t: TFn): string {
  const s = d.toString();
  const date = new Date(parseInt(s.slice(0, 4)), parseInt(s.slice(4, 6)) - 1, parseInt(s.slice(6, 8)));
  ...
  return t('inDays', { n: diff });
}

function resolveName(raw: unknown, locale: Locale): string {
  if (!raw) return '';
  if (typeof raw === 'string') return raw;
  const obj = raw as Record<string, string>;
  return obj[locale] ?? obj.de ?? obj.it ?? obj.en ?? String(raw);
}
...
function SectionHeader({ title, href }: { title: string; href?: string }) {
  ...
  return (
    <div className="flex items-center justify-between mb-3">
      ...
      {href && (
        <Link href={localize(href)} className="flex items-center gap-1 text-xs font-medium transition-opacity hover:opacity-70" style={{ color: 'var(--accent)' }}>
...
function StatCard({ label, value, sub, color, icon: Icon, href }: { label: string; value: string; sub?: string; color: string; icon: React.ComponentType<{ size?: number; strokeWidth?: number }>; href?: string }) {
  ...
  if (href) return <Link href={localize(href)} className="block press-scale">{inner}</Link>;
  return inner;
}
...
export default function HomePage() {
  ...
  const load = useCallback(async () => {
    ...
    if (staleTt !== undefined || staleGr !== undefined || staleMs !== undefined) {
      ...
      if (staleMs !== undefined) setMessages(parseMessagesResult(staleMs, tm('unknown'), tm('noSubject')));
      setLoading(false);
    }
    ...
    try {
      ...
      if (msRes.status === 'fulfilled') setMessages(parseMessagesResult(msRes.value, tm('unknown'), tm('noSubject')));
      setLoading(false);
          ...
          <div className="max-w-6xl mx-auto px-4 md:px-6 py-6">
            ...
            <div className="mb-6 fade-in">
              <p className="text-sm mb-1" style={{ color: 'var(--app-text-secondary)' }}>
                {formatDate(new Date(), tag, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
              </p>
            ...
            {loading ? (
              ...
              <div className="flex flex-col gap-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 fade-in">
                  <StatCard label={t('lessonsEnd')} value={lastActive ? parseTime(lastActive.endTime) : '–'} sub={firstActive ? t('startAt', { time: parseTime(firstActive.startTime) }) : t('noLessons')} color="var(--accent)" icon={Clock} />
                  <StatCard label={t('gradeAvg')} value={overallAvg != null ? overallAvg.toFixed(2) : '–'} sub={subjectCount === 1 ? t('subjectOne', { n: subjectCount }) : t('subjectMany', { n: subjectCount })} color={overallAvg != null ? averageColor(overallAvg) : 'var(--app-text-secondary)'} icon={TrendingUp} href="/grades" />
                  <StatCard label={t('lessonsToday')} value={String(countPeriods(activeTodayEntries))} sub={activeTodayEntries.length !== todayEntries.length ? t('activeCount', { n: activeTodayEntries.length }) : undefined} color="var(--tint)" icon={BookOpen} href="/timetable" />
                  <StatCard label={t('messages')} value={String(messages.length)} sub={unreadCount > 0 ? t('unreadCount', { n: unreadCount }) : t('allRead')} color={unreadCount > 0 ? 'var(--danger)' : 'var(--app-text-secondary)'} icon={MessageCircle} href="/messages" />
                </div>

                {dishes.length > 0 && (
                  <section className="fade-in">
                    <SectionHeader title={t('mensaToday')} href="/mensa" />
                    <div className="flex flex-col gap-2.5">
                      {dishes.map((dish) => {
                        const name = resolveName(dish.name, locale);
                        const desc = dish.description ? resolveName(dish.description, locale) : null;
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <section className="fade-in">
                    <SectionHeader title={t('todayWithDate', { date: formatDate(new Date(), tag, { day: 'numeric', month: 'long' }) })} href="/timetable" />
                    <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                      {todayEntries.length === 0 ? (
                        <div className="px-4 py-6 text-center">
                          <p className="text-sm" style={{ color: 'var(--app-text-secondary)' }}>{t('noLessonsToday')}</p>
                        </div>
                      ) : (
                        todayEntries.map((e, i) => {
                          ...
                          return (
                            <Link key={e.id} href={localize(`/timetable?open=${e.id}`)} className="flex items-center gap-3 px-4 py-3 press-scale" style={{ borderTop: i > 0 ? '1px solid var(--app-separator)' : 'none', display: 'flex' }}>
                              ...
                              <div className="flex gap-1.5 flex-shrink-0">
                                {e.isExam && !e.isCancelled && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: 'color-mix(in srgb, var(--warning) 20%, transparent)', color: 'var(--warning)' }}>{t('exam')}</span>}
                                {e.isCancelled && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: 'color-mix(in srgb, var(--danger) 15%, transparent)', color: 'var(--danger)' }}>{t('cancelled')}</span>}
                                {e.isSubstitution && !e.isCancelled && <span className="text-[10px] font-semibold px-2 py-0.5 rounded-full" style={{ background: 'color-mix(in srgb, var(--orange) 20%, transparent)', color: 'var(--orange)' }}>{t('substitution')}</span>}
                              </div>
                            </Link>
                          );
                        })
                  ...
                  <div className="flex flex-col gap-6">
                    ...
                    {nextExam && (
                      <section className="fade-in">
                        <SectionHeader title={t('nextExam')} href="/timetable" />
                        <Link href={localize(`/timetable?open=${nextExam.id}&date=${nextExam.date}`)} className="block rounded-2xl p-4 press-scale" style={{ background: 'var(--app-surface)', border: '1px solid color-mix(in srgb, var(--warning) 35%, var(--app-border))' }}>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-xs mt-0.5" style={{ color: 'var(--app-text-secondary)' }}>
                                {formatExamDate(nextExam.date, tag)} · {parseTime(nextExam.startTime)}{nextExam.roomName ? ` · ${nextExam.roomName}` : ''}
                              </p>
                            </div>
                            <span className="text-xs font-bold px-2.5 py-1 rounded-full flex-shrink-0" style={{ background: 'color-mix(in srgb, var(--warning) 18%, transparent)', color: 'var(--warning)' }}>
                              {daysUntilLabel(nextExam.date, t)}
                            </span>
                          </div>
                        </Link>
                      </section>
                    ...
                    {recentGrades.length > 0 && (
                      <section className="fade-in">
                        <SectionHeader title={t('recentGrades')} href="/grades" />
                        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                          {recentGrades.map((g, i) => (
                            <Link key={g.id} href={localize(`/grades/subject/${g.subjectLessonId}`)} className="px-4 py-3 flex items-center gap-3 press-scale" style={{ borderTop: i > 0 ? '1px solid var(--app-separator)' : 'none', display: 'flex' }}>
                              <div className="flex-1 min-w-0">
                                <p className="text-sm font-medium truncate" style={{ color: 'var(--app-text-primary)' }}>{g.subjectName || '–'}</p>
                                <p className="text-xs" style={{ color: 'var(--app-text-tertiary)' }}>{g.examType || t('grade')} · {formatGradeDate(g.date)}</p>
                              </div>
                    ...
                    {messages.length > 0 && (
                      <section className="fade-in">
                        <SectionHeader title={t('messages')} href="/messages" />
                        <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--app-surface)', border: '1px solid var(--app-border)' }}>
                          {messages.slice(0, 3).map((msg, i) => (
                            <Link key={msg.id} href={localize(`/messages/${msg.id}`)} className="px-4 py-3 flex items-center gap-3 press-scale" style={{ borderTop: i > 0 ? '1px solid var(--app-separator)' : 'none', display: 'flex' }}>
                              ...
                              <div className="flex items-center gap-2 flex-shrink-0">
                                <p className="text-xs" style={{ color: 'var(--app-text-tertiary)' }}>{formatMessageDate(msg.sentDate, tag, t('yesterday'))}</p>
