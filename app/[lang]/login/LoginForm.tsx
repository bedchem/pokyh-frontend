...
export default function LoginForm() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    setLoading(true);
    setLoadingMsg(t('loggingIn'));
    setError('');

    const raw = params.get('next') ?? '';
    const next = raw.startsWith('/') && !raw.startsWith('//') ? raw : localize('/home');
    ...
    try {
      ...
      setError(untisData.error ?? pokyhData.error ?? t('errFailed'));
      setLoading(false);
      setLoadingMsg('');
    } catch (e: unknown) {
  ...
  return (
    <div className="min-h-dvh flex">
      ...
      >
        ...
        {/* Logo */}
        <Link href={localize('/')} className="flex items-center gap-3 relative z-10 transition-opacity hover:opacity-80">
          <div style={{
        ...
        <div className="flex flex-col gap-2.5 relative z-10">
          {[t('f1'), t('f2'), t('f3'), t('f4'), t('f5')].map((f) => (
      ...
      {/* ── RIGHT PANEL — form ───────────────────────────────────────────── */}
      <div className="relative flex-1 flex flex-col items-center justify-center px-6 py-12 min-h-dvh">
        ...
        <div className="w-full max-w-[360px]">
          ...
          <form onSubmit={handleLogin} className="flex flex-col" noValidate>
            ...
            >
              {loading || autoFilling
                ? <><Spinner size={18} /><span>{loadingMsg || t('loggingIn')}</span></>
                : t('login')}
            </button>
          </form>
          ...
          >
            ...
            <p className="text-[12px] leading-relaxed" style={{ color: 'var(--app-text-tertiary)' }}>
              ...
              {t('hintAfter').startsWith('.') ? '' : ' '}{t('hintAfter')}
            </p>
          </div>

          {/* Legal */}
          <div className="flex items-center justify-center gap-3 mt-6">
            <Link href={localize('/legal?view=impressum')} className="text-xs transition-opacity hover:opacity-70" style={{ color: 'var(--app-text-tertiary)' }}>
              {t('imprint')}
            </Link>
            <span style={{ color: 'var(--app-text-tertiary)' }} className="text-xs">·</span>
            <Link href={localize('/legal?view=datenschutz')} className="text-xs transition-opacity hover:opacity-70" style={{ color: 'var(--app-text-tertiary)' }}>
              {t('privacy')}
            </Link>
          </div>
