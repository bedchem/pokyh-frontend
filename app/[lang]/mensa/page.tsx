...
function groupDishes(dishes: Dish[]): GroupedDishes[] {
  ...
  map.forEach((dayDishes, key) => {
    ...
    result.push({ date, label: key, isToday: todayFlag, isTomorrow: tomorrowFlag, dishes: dayDishes });
  });
      ...
      >
        ...
        <div className="relative" style={{ height: 200 }}>
          {dish.imageUrl ? (
            ...
            <img
              src={dish.imageUrl}
              alt={resolveName(dish.name, locale)}
              className="w-full h-full object-cover"
            />
          ) : (
            <div className="w-full h-full" style={{ background: dishPlaceholderBg(dish) }} />
        ...
        <div className="p-5">
          <div className="mb-2">
            <h2 className="text-xl font-bold" style={{ color: 'var(--app-text-primary)' }}>
              {resolveName(dish.name, locale)}
            </h2>
          </div>

          {dish.description && (
            <p className="text-sm mb-3" style={{ color: 'var(--app-text-secondary)' }}>
              {resolveName(dish.description, locale)}
            </p>
          )}
          ...
          <div className="rounded-xl p-4 mb-4" style={{ background: 'var(--app-card)' }}>
            ...
            {ratingCount > 0 && (
              <p className="text-xs mt-2" style={{ color: 'var(--app-text-tertiary)' }}>
                {ratingCount === 1 ? t('ratingOne') : t('ratingMany', { n: ratingCount })}
              </p>
            )}
          </div>
          ...
          {(dish.calories != null || dish.protein != null || dish.carbs != null || dish.fat != null) && (
            <div className="rounded-xl p-4 mb-4 grid grid-cols-4 gap-3" style={{ background: 'var(--app-card)' }}>
              ...
              {dish.protein != null && <NutriCell label={t('protein')} value={`${dish.protein}g`} />}
              {dish.carbs != null && <NutriCell label={t('carbs')} value={`${dish.carbs}g`} />}
              {dish.fat != null && <NutriCell label={t('fat')} value={`${dish.fat}g`} />}
            </div>
          )}
...
export default function MensaPage() {
  ...
  // Client page without its own metadata: keep the tab title in the current language.
  useEffect(() => {
    document.title = `${t('title')} | POKYH`;
  }, [t]);
  ...
  const header = (
    <div className="px-5 pt-5 pb-4 fade-in flex-shrink-0">
      ...
      <p className="text-sm mt-0.5" style={{ color: 'var(--app-text-secondary)' }}>{t('school')}</p>
    </div>
  );
                  ...
                  <div className="flex items-center gap-2 mb-3 px-1">
                    ...
                    >
                      {group.isToday
                        ...
                        : group.isTomorrow
                          ? t('tomorrow')
                          : formatDate(group.date, tag, { weekday: 'long', day: 'numeric', month: 'long' })}
                    </h2>
                    {group.isToday && (
                      <span
                        className="text-[10px] font-semibold px-2 py-0.5 rounded-full"
