    ...
    <div className={s.grid} style={{ ['--s' as string]: scale } as CSSProperties}>
      ...
      <div className={s.gridHeader} style={{ gap: colGap }}>
        <div className={s.gutterHead} style={{ width: gutter }}>
          <span className={s.gutterKw}>KW {weekNumber}</span>
          {dates[0] && <span className={s.gutterMonth}>{monthShortDe(dates[0])}</span>}
        </div>
...
function GridLessonCell({ cell, height, width, dense, scale, onTap }: {
  ...
  const label = [subjectText, entry.teacherName, entry.roomName, cancelled ? 'Entfall' : ''].filter(Boolean).join(', ');
...
export function WeekGridSkeleton() {
  ...
  return (
    <div className={s.skelGrid} aria-busy="true" aria-label="Stundenplan wird geladen">
